...
db.once 'open', ->
	...
	bot.addListener 'message', (from, to, message) ->
		...
		if message.match /^\.bots/
			...
			if modules[command] then modules[command].func message, from, (x) -> bot.say to, x
			else if alias[command] then alias[command].func message, from, (x) -> bot.say to, x
		else if message.match /^\?./
			split = message.split ' '
			command = split[0].substring 1
	...
	bot.addListener 'pm', (from,	message) ->
		...
		if message.match /^\.help/
			...
			command = split[0].substring 1
			args = split[1..]
			if modules[command] then modules[command].func message, from, (x) -> bot.say from, x
			else if alias[command] then alias[command].func message, from, (x) -> bot.say from, x
		else if message.match /^\?./
			split = message.split ' '
			command = split[0].substring 1
